...
def push_profiling_event(name: str,  start_time: float, end_time: float, thread_id: Optional[str] = None):
    ...
    PROFILING_EVENTS_QUEUE.put((name, thread_id, start_time-PROFILING_T0, end_time-PROFILING_T0))
...
def export_profiling_events(output_path: str):
    ...
    while not PROFILING_EVENTS_QUEUE.empty():
        ...
        events.append({"name": name, "ph": "B", "ts": t_begin*1e6, "tid": tid, "pid": 0})
        events.append({"name": name, "ph": "E", "ts": t_end*1e6, "tid": tid, "pid": 0})
    ...
    print(f"Open Chrome, type chrome://tracing/ and load the file located at {os.path.abspath(output_path)}")
