...
def _read_lines(path):
    with open(path) as f:
        ...
        return Amphipods(dico['A'], dico['B'], dico['C'], dico['D'])
...
class Amphipods:
    def __init__(self,
                 ...
                 cc: Tuple[int, int],
                 dd: Tuple[int, int]):
        self.amphis: List[int] = [max(aa), min(aa), max(bb), min(bb), max(cc), min(cc), max(dd), min(dd)]
        self.finished: List[bool] = [False for _ in range(8)]

        for idx, pos in enumerate(self.amphis):
            family = idx_to_family(idx)
            if pos == family*10+2:
                self.finished[idx] = True
            elif pos == family*10+1 and idx % 2 == 1 and self.finished[idx-1]:
                self.finished[idx] = True

    def solve(self) -> int:
        dp = {}
        return propagate(self.amphis, self.finished, dp, [])


def part_one(path: str) -> int:
    amphipods = _read_lines(path)
    return amphipods.solve()
